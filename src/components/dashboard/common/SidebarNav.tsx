...
export function SidebarNav({ role, isOpen, onToggle }: SidebarNavProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed left-0 top-0 z-50 h-full bg-[var(--dashboard-sidebar)] border-r border-[var(--dashboard-border)] transition-all duration-300 ease-in-out',
          isOpen ? 'w-64' : 'w-20',
          'lg:translate-x-0',
      ...
      >
        ...
        <nav className="flex-1 space-y-1 p-3">
          {navItems.map((item) => {
            ...
            // More precise active check - don't use startsWith for dashboard root
            const isRootDashboard = item.href === '/dashboard' || item.href === '/admin/dashboard';
