...
export default function UserDashboardPage() {
  ...
  const totalTop10rankings = mockRankCheckHistory.reduce(
    (sum, item) => sum + item.top10Count,
    0,
  );
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-2xl font-bold ${getScoreColor(avgSEOScore)}`}
                  >
                    {avgSEOScore}
                  </span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-center">
                  ...
                  <div className="rounded-lg border p-2">
                    <p className="text-lg font-bold text-red-600">
                      {mockSEOAnalysisHistory.reduce(
                        (sum, i) => sum + i.criticalIssues,
                        0,
                      )}
                    </p>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold">
                    {totalKeywordsResearched.toLocaleString()}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-2 text-center">
                  <div className="rounded-lg border p-2">
                    <p className="text-lg font-bold text-emerald-600">
                      {
                        mockKeywordResearchHistory.filter(
                          (i) => i.status === "completed",
                        ).length
                      }
                    </p>
                    ...
                    <p className="text-lg font-bold">
                      {Math.round(
                        mockKeywordResearchHistory
                          .filter((i) => i.status === "completed")
                          .reduce(
                            (sum, i) => sum + i.relatedKeywordsCount,
                            0,
                          ) /
                        mockKeywordResearchHistory.filter(
                          (i) => i.status === "completed",
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-center">
                  <div className="rounded-lg border p-2">
                    <p className="text-lg font-bold">
                      {mockRankCheckHistory.reduce(
                        (sum, i) => sum + i.keywords.length,
                        0,
                      )}
                    </p>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Your SEO performance over the last 7 days
                </CardDescription>
              </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Recent issues and opportunities
              </CardDescription>
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {recentSEOAnalyses.map((analysis) => (
              ...
              >
                <div
                  className={`rounded-full p-2 ${analysis.status === "completed" ? "bg-emerald-100 dark:bg-emerald-900/30" : "bg-amber-100 dark:bg-amber-900/30"}`}
                >
                  <Globe
                    className={`h-4 w-4 ${analysis.status === "completed" ? "text-emerald-600" : "text-amber-600"}`}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {recentKeywordResearch.map((research) => (
              ...
              >
                <div
                  className={`rounded-full p-2 ${research.status === "completed" ? "bg-violet-100 dark:bg-violet-900/30" : "bg-amber-100 dark:bg-amber-900/30"}`}
                >
                  <Lightbulb
                    className={`h-4 w-4 ${research.status === "completed" ? "text-violet-600" : "text-amber-600"}`}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {recentRankChecks.map((check) => (
              ...
              >
                <div
                  className={`rounded-full p-2 ${check.status === "completed" ? "bg-amber-100 dark:bg-amber-900/30" : "bg-gray-100 dark:bg-gray-800"}`}
                >
                  <Target
                    className={`h-4 w-4 ${check.status === "completed" ? "text-amber-600" : "text-gray-600"}`}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">
